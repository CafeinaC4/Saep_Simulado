...
app.post('/movimentacoes', async (req, res) => {
  const { material_id, usuario_id, tipo, quantidade, data_movimentacao, observacao } = req.body || {};
  ...
  try {
    ...
    const ins = await client.query(
      `INSERT INTO movimentacoes (material_id, usuario_id, tipo, quantidade, data_movimentacao, observacao)
       VALUES ($1,$2,$3,$4,COALESCE($5, NOW()),$6)
       RETURNING id, material_id, usuario_id, tipo, quantidade, data_movimentacao, observacao`,
      [material_id, usuario_id, String(tipo).toLowerCase(), Math.abs(quantidade), data_movimentacao || null, observacao || null]
    );
...
app.get('/movimentacoes', async (req, res) => {
  ...
  const sql = `
    SELECT m.id, m.material_id, p.nome AS material_nome,
